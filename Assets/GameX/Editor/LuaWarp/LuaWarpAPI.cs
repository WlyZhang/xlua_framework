...
/*
 * 1. 该文件是xLua的配置文件，主要用于生成Lua绑定代码。
 * 2. 需要在Unity编辑器中运行时生成Lua绑定代码。
 * 3. 配置包括Lua调用C#的类型、C#调用Lua的委托和接口、黑名单等。
 * 4. 可以根据项目需求修改配置内容。
 * 5. 此文件用于备份，如果需要使用，请将其复制到项目的Assets/xLua/Examples目录下，并重命名为ExampleGenConfig.cs。
*/
...
public static class ExampleGenConfig
{
    ...
    public static List<Type> LuaCallCSharp = new List<Type>() {
                ...
                typeof(CXConfig),
                typeof(CXEngine),
                typeof(DirectoryType),
                typeof(Lua),
                typeof(AssetLoadManager),
                typeof(CompileLibrary),
                typeof(FingerEvent),
                typeof(HttpCenter),
                typeof(Tweener),
                typeof(JoyStick),
                typeof(FingerGestures.SwipeDirection),
                typeof(FingerEvent),
                typeof(QR),
